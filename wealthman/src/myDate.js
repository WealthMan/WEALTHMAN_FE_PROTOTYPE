  ...
  constructor(date, lang) {
    if (typeof date === 'undefined')
      ...
      if (date[2] == '.' || date[2] == '/' || date[2] == '-')
        this.dateJS = new Date(date.substring(6, 10) + '.' + date.substring(3, 5) + '.' + date.substring(0, 2));
      else if (date[2] == ':' && (date[8] == '.' || date[8] == '/' || date[8] == '-'))
        this.dateJS = new Date(date.substring(0, 6) + date.substring(12, 16) + '.' + date.substring(9, 11) + '.' + date.substring(6, 8));
      else
        this.dateJS = new Date(date);
    else if (typeof date === 'number')
      this.dateJS = new Date(date);
    else if (typeof date === 'object')
      this.dateJS = new Date(date.dateJS);
    else
      alert('unexpected typeof date: ' + typeof date);
